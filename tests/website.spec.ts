...
test.describe("Website Management", () => {
  ...
  test.describe.serial("Add Content panel shows English, not locale keys", () => {
    // Creates its own page — a retry would collide on the URL slug.
    test.describe.configure({ retries: 0 });
    ...
    test("element cards render translated text", async () => {
      // The /site/pages route compiles on first hit; wait it out before interacting.
      const addPageBtn = page.locator('[data-testid="add-page-button"]');
      await expect(addPageBtn).toBeVisible({ timeout: 30000 });
      ...
      // Wait for the Add Page dialog to fully close, otherwise its backdrop
      // intercepts the edit-content click.
      await expect(page.locator('[name="title"]')).toHaveCount(0);
      await row.locator('[data-testid="edit-content-button"]').click();
