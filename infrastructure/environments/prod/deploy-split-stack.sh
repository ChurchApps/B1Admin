...
echo "Auditing prod starter files..."
run_starter_audit

echo "Planning prod deployment readiness..."
run_deploy_plan
...
echo "Deploying split-stack prod environment..."
run_npm deploy:aws -- "${DEPLOY_ARGS[@]}"

if [[ "${SAVE_OUTPUTS_AFTER_DEPLOY}" == "true" ]]; then
  echo "Saving split-stack prod outputs..."
...
if [[ "${VERIFY_AFTER_DEPLOY}" == "true" ]]; then
  echo "Verifying split-stack prod environment..."
  VERIFY_ARGS=(
    --region="${AWS_REGION}"
