...
import { Plate, h1Sx, ledeSx, addBarSx, verbSx } from "./plated";
...
export const EmailTemplatesPage: React.FC = () => {
  ...
  return (
    <Plate directory>
      ...
      <Box component="h1" sx={h1Sx}>{Locale.label("settings.emailTemplatesPage.title")}</Box>
      <Box sx={ledeSx}>{Locale.label("settings.emailTemplatesPage.subtitle")}</Box>
      {editTemplate !== null && (
        <Box sx={{ mb: 3 }}>
      ...
      {editTemplate === null && (
        <Box sx={addBarSx}>
          <Box component="button" type="button" onClick={handleNew} sx={verbSx}>
